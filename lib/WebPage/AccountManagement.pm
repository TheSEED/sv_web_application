...
x.style.display = 'none';
  ...
  if ($user->has_right(undef, 'impersonate', 'user', '*')) {
      ...
      foreach $user (sort { (lc($a->lastname()) cmp lc($b->lastname())) ||
				(lc($a->firstname()) cmp lc($b->firstname())) ||
				(lc($a->login()) cmp lc($b->login()))
			    } @$users_to_impersonate) 
      {
	  $user_select .= "<option value=\'" . $user->login() . "\'>" 
