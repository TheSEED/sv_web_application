...
sub output {
  ...
  #  open(L, ">/tmp/log");
  # print L "POP: " . Dumper(\@jobs, $jobs_hash, $genomes);
  ...
  foreach my $pref ( @$prefs ) {
      # print L "CHECK " . $pref->value . "\n";
    if ($genomes->{$pref->value}) {
	# print L "ADD " . $pref->value . "\n";
  ...
  my $availables = [];
  foreach my $job (sort { $b->{id} <=> $a->{id} } @jobs) {
...
sub set_peers {
  ...
  foreach my $j (@new_peers) {
    ...
    if (ref($job)) {
      ...
      $genome_to_job{$job->genome_id} = $job->id;
    }
  }
  ...
  # pref_id is a genome id
  # jobs_hash maps from job id to job
  
  foreach my $pref_id (keys(%peer_prefs_hash)) {
      my $pjob = $genome_to_job{$pref_id};
      unless ($new_peers_hash{$pjob}) {
	  # print L "DELETE pref_id=$pref_id jh=$jobs_hash->{$pjob} pjob=$pjob \n";
  ...
  foreach my $peer (@new_peers) {
      my $newg = $genomes_hash->{$peer};
      #print L "CHECK peer=$peer newg=$newg\n";
      
    unless (exists($peer_prefs_hash{$newg})) {
	#print L "ADD $newg $peer\n";
...
sub check_requirements {
  ...
  foreach my $job1 (@peers) {
	...
	push(@$row, { data => $status,
		      highlight => $color,
		      tooltip => $jobs_hash->{$job1}->genome_id . q(-) . $jobs_hash->{$job2}->genome_id 
	     });
      }
    }
    push(@$sim_data, $row);
